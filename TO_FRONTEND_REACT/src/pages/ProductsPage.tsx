...
// ✅ Tuzatishlar:
// 1. productsApi.getAll → { data, total, pageCount } formatini to'g'ri o'qiydi
// 2. Rasm URL tuzatildi (fixImageUrl product.api.ts da)
// 3. Pagination ishlaydi
  ...
  const wsData = [
    ...
    ...products.map((p, i) => {
      ...
      return [i+1, p.name, p.category?.name ?? 'Kategoriyasiz',
        ...
        pur, sal, Number(p.stockQuantity), Number(p.minStockLimit ?? 0), status, `${margin}%`];
    }),
    ['JAMI', `${products.length} ta`, '', '', '', '',
     products.reduce((s, p) => s + Number(p.stockQuantity), 0), '',
     `${products.filter(p => p.stockQuantity <= 0).length} tugagan`, ''],
  ];
  const ws = XLSX.utils.aoa_to_sheet(wsData);
  ws['!cols'] = [{wch:5},{wch:35},{wch:18},{wch:10},{wch:14},{wch:14},{wch:10},{wch:12},{wch:14},{wch:10}];
...
export function ProductsPage() {
  ...
  // ✅ productsApi.getAll endi to'g'ri ProductsResponse qaytaradi
  const { data: response, isLoading } = useQuery({
    queryKey: ['products', page, search],
    queryFn:  () => productsApi.getAll(page, search || undefined),
    staleTime: 5 * 60_000,
  });
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      const { data } = await api.get('/products', {
        params: { page: 1, limit: 9999, ...(search ? { search } : {}) },
      });
  ...
  const lowStockCount   = products.filter(p => p.stockQuantity > 0 && p.stockQuantity <= (p.minStockLimit ?? 5)).length;
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-700
                       disabled:opacity-60 text-white rounded-xl text-sm font-semibold transition-all"
          >
            {isExporting
          ...
          {isAdmin && (
            <Button leftIcon={<Plus size={16} />} onClick={() => { setEditingProduct(null); setIsFormOpen(true); }}>
              Mahsulot qo&apos;shish
            </Button>
          )}
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {products.length === 0 ? (
                ...
                const isLow     = product.stockQuantity > 0 && product.stockQuantity <= (product.minStockLimit ?? 5);
                ...
                return (
                  <TableRow key={product.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-lg bg-gray-100 flex items-center justify-center
                                        overflow-hidden border border-gray-200 flex-shrink-0">
                          {product.imageUrl ? (
                            <img
                        ...
                        <span className="font-medium text-gray-900 min-w-0 truncate max-w-[180px]">
                          {product.name}
                        </span>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="font-medium text-indigo-600 tabular-nums">
                    ...
                    <TableCell>
                      {product.stockQuantity > 0 ? (
                        <span className={`font-medium tabular-nums ${isLow ? 'text-amber-600' : 'text-gray-700'}`}>
                          {parseFloat(String(product.stockQuantity))}{' '}
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {Array.from({ length: Math.min(5, pageCount) }, (_, i) => {
                let p = pageCount <= 5 ? i + 1
                  : page <= 3 ? i + 1
                  : page >= pageCount - 2 ? pageCount - 4 + i
                  : page - 2 + i;
                return (
      ...
      <ProductFormModal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} productToEdit={editingProduct} />
