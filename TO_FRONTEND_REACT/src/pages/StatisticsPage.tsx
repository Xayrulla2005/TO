...
import { ArrowUpRight, TrendingUp, PieChart as PieIcon, BarChart3 } from 'lucide-react';
    ...
    <div className="flex items-end justify-between h-full gap-2 pt-6">
      {data.map((point, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end">
          ...
          <span className="text-xs text-gray-500 rotate-0 truncate max-w-[60px]">{point.label}</span>
        </div>
      ))}
    </div>
  );
        ...
        <Card>
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 mt-2">{formatCurrency(data?.revenue || 0)}</h3>
            </div>
        ...
        <Card>
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 mt-2">{formatCurrency(data?.profit || 0)}</h3>
            </div>
        ...
        <Card>
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 mt-2">{data?.ordersCount || 0}</h3>
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <PieIcon size={18} /> Payment Split
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            {[
              { label: 'Cash', value: data?.paymentSplit.cash || 0, color: 'bg-green-500' },
              { label: 'Card', value: data?.paymentSplit.card || 0, color: 'bg-blue-500' },
              { label: 'Debt', value: data?.paymentSplit.debt || 0, color: 'bg-red-500' },
            ].map((item) => {
              ...
              return (
                <div key={item.label}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-gray-500">{percent}% ({formatCurrency(item.value)})</span>
                  </div>
