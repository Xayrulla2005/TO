// src/pages/SalesPage.tsx — TO'LIQ TUZATILGAN
// Tuzatishlar:
// 1. productsApi.getAll → to'g'ri ProductsResponse formatini o'qiydi
// 2. salesApi.create → data?.data ?? data wrapper
// 3. Rasmlar to'liq URL bilan ko'rsatiladi
// 4. Savat LocalStorage da saqlanadi (sahifa yangilanishda yo'qolmaydi)
// 5. Mahsulot qidiruv — debounce bilan
// 6. Savat yangilanishida agreedPrice reset bo'ladi
...
import {
  Search, ShoppingCart, Trash2, Package,
  X, ChevronUp, AlertTriangle, RefreshCw,
} from "lucide-react";
...
export function SalesPage() {
  ...
  useEffect(() => { receiptUrlRef.current = receiptUrl; }, [receiptUrl]);
  useEffect(() => {
    return () => { if (receiptUrlRef.current) URL.revokeObjectURL(receiptUrlRef.current); };
  }, []);

  const closeReceipt = () => {
    if (receiptUrl) setTimeout(() => URL.revokeObjectURL(receiptUrl), 150);
    setReceiptUrl(null);
  };

  // ── Mahsulotlarni yuklash ─────────────────────────────────
  // ✅ To'g'ridan api.get — PaginatedResponseDto formatini o'qiymiz
  const { data: productsRaw, isLoading: productsLoading, refetch } = useQuery({
    queryKey: ["products-sales"],
    queryFn:  async () => {
      const { data } = await api.get("/products", {
        params: { page: 1, limit: 1000 },
      });
      // Backend: { data: [], meta: { totalItems } } yoki { data: [], total }
      const list: any[] =
  ...
  const subtotal = useMemo(
    () => cart.reduce((sum, i) => safeAdd(sum, safeMultiply(i.qty, i.unitPrice)), 0),
    [cart],
  );
  const originalSubtotal = useMemo(
    () => cart.reduce((sum, i) => safeAdd(sum, safeMultiply(i.qty, Number(i.product.salePrice) || 0)), 0),
    [cart],
  );
  ...
  const totalItems = useMemo(
    () => cart.reduce((sum, i) => safeAdd(sum, i.qty), 0),
    [cart],
  );
  ...
  const createSaleMutation = useMutation({
    mutationFn: async (paymentData: PaymentData) => {
      // 1-qadam: DRAFT yaratish
      const draftRaw = await salesApi.create({
        items: cart.map((item) => ({
      ...
      // ✅ Backend wrapper: { data: {...} } yoki to'g'ridan object
      const draft = (draftRaw as any)?.data ?? draftRaw;
      if (!draft?.id) throw new Error("DRAFT savdo yaratilmadi");
      ...
      // 3-qadam: Complete
      const completedRaw = await salesApi.complete(draft.id, {
        payments,
      ...
      return (completedRaw as any)?.data ?? completedRaw;
    },
    ...
    onError: (e: any) => {
      const msg = e?.response?.data?.message ?? e?.message ?? "Savdoni yakunlashda xatolik";
      toast.error(msg);
    },
  });
  ...
  const updatePrice = useCallback((productId: string, price: number) => {
    ...
    setCart((prev) =>
      prev.map((i) => i.product.id === productId ? { ...i, unitPrice: safePrice } : i),
    );
    setAgreedPrice(null);
  }, []);
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 mb-3 sticky top-0 z-10">
            <div className="flex gap-2">
              <div className="flex-1">
                <Input
                  placeholder="Mahsulotni nomi bo'yicha qidiring..."
                  icon={<Search size={16} />}
                  value={search}
              ...
              <button
                onClick={() => refetch()}
                className="p-2 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors flex-shrink-0"
            ...
            <div className="flex gap-2 mt-2 overflow-x-auto pb-1 scrollbar-hide">
              ...
              >
                Barchasi {allProducts.length > 0 && `(${allProducts.length})`}
              </button>
          ...
          <div className="flex-1 overflow-y-auto space-y-1.5">
            {filteredProducts.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-48 text-gray-400">
                ...
                <p className="text-sm">
                  {allProducts.length === 0 ? "Mahsulotlar yuklanmagan" : "Mahsulot topilmadi"}
                </p>
              ...
              filteredProducts.map((product: Product) => {
                ...
                const isOut  = stock <= 0;
                const isLow  = stock > 0 && stock <= (product.minStockLimit ?? 5);
          ...
          <Card className="flex flex-col h-full border-indigo-100 shadow-lg">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {cart.length > 0 && (
                  <button
                    onClick={() => { setCart([]); setAgreedPrice(null); }}
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                  ...
                  <div className="flex items-center gap-2">
                    {cart.length > 0 && (
                      <button
                        onClick={() => { setCart([]); setAgreedPrice(null); }}
                        className="text-xs text-red-400 px-2 py-1 rounded-lg hover:bg-red-50 transition-colors"
                      >
                        Tozalash
                      </button>
                    )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          ...
          >
            <div className="p-4 border-b flex justify-between items-center">
              <div>
                <h3 className="font-bold text-gray-900">Savdo cheki</h3>
                <p className="text-xs text-gray-400 mt-0.5">#{completedSale.saleNumber}</p>
              </div>
              <button onClick={closeReceipt} className="p-1.5 hover:bg-gray-100 rounded-lg">
                <X size={18} className="text-gray-500" />
              </button>
            </div>

            {/* Qarz xabari */}
            {completedSale.debtSummary && (completedSale.debtSummary as any).currentSaleDebt > 0 && (
              <div className="mx-4 mt-3 bg-amber-50 border border-amber-200 rounded-xl p-3">
                <p className="text-xs font-semibold text-amber-700">
                  Nasiya: {fmt((completedSale.debtSummary as any).currentSaleDebt)}
                  {(completedSale.debtSummary as any).previousDebt > 0 && (
                    <span className="ml-2 text-amber-500">
                      (Jami qarz: {fmt((completedSale.debtSummary as any).totalDebtAfter)})
                    </span>
                  )}
                </p>
              </div>
            ...
            <div className="p-3 border-t flex gap-2">
              ...
              <a
                href={receiptUrl}
                download={`chek-${completedSale.saleNumber}.pdf`}
                className="flex-1 bg-gray-100 text-gray-700 text-center py-2.5 rounded-xl font-semibold text-sm hover:bg-gray-200 transition-colors"
              >
                PDF yuklash
              </a>
            </div>
...
function CartItemRow({
  ...
  const lineTotal = useMemo(
    () => safeMultiply(item.qty, item.unitPrice),
    ...
    <div className="bg-gray-50 rounded-xl border border-gray-100 p-2.5 space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <p className="text-xs text-gray-400">Asl: {fmt(originalPrice)}</p>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={() => { const v = Math.max(0.001, item.qty - 1); onUpdateQty(item.product.id, v); }}
            className="w-6 h-6 rounded-md bg-gray-200 text-gray-600 font-bold text-sm flex items-center justify-center hover:bg-gray-300 transition-colors"
          >
            −
          </button>
          <input
            type="number" step="any" value={qtyInput}
            onChange={(e) => {
            ...
            className="w-14 px-1 py-1 text-xs border border-gray-200 rounded-lg focus:outline-none focus:border-indigo-400 text-center bg-white font-bold"
            min={0.001}
          />
          <button
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative">
            ...
            <input
              type="number" step="any" value={priceInput}
              onChange={(e) => {
                const raw = e.target.value;
                setPriceInput(raw);
                const val = parseFloat(raw);
...
function CartFooter({
  ...
  return (
    <div className="p-3 border-t border-gray-100 bg-white space-y-2">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            type="text"
            inputMode="decimal"
            defaultValue={agreedPrice !== null ? String(agreedPrice) : String(subtotal)}
            onChange={(e) => {
              const raw = e.target.value;
              if (!/^[\d.]*$/.test(raw)) return;
              if ((raw.match(/\./g) || []).length > 1) return;
              if (raw === "") { onAgreedPriceChange(null); return; }
              if (raw.endsWith(".")) { onAgreedPriceChange(raw); return; }
              const val = Number(raw);
              if (isNaN(val) || val < 0) return;
              onAgreedPriceChange(Math.abs(val - subtotal) < 0.0001 ? null : val);
            }}
      ...
      >
        {cart.length === 0 ? "Savat bosh" : `To'lov — ${fmt(displayTotal)}`}
      </Button>
    </div>
  );
}