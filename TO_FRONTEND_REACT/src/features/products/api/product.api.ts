...
export const productsApi = {
  ...
  create: async (values: ProductFormValues) => {
    ...
    formData.append('stockQty', values.stockQty.toString());
    if (values.image) {
  ...
  update: async (id: string, values: Partial<ProductFormValues>) => {
    ...
    if (values.purchasePrice !== undefined) formData.append('purchasePrice', values.purchasePrice.toString());
    if (values.salePrice !== undefined) formData.append('salePrice', values.salePrice.toString());
    if (values.stockQty !== undefined) formData.append('stockQty', values.stockQty.toString());
    if (values.image) {
      formData.append('image', values.image);
    }
